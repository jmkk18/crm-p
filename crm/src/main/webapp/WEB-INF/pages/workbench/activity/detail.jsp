...
<script type="text/javascript">
	...
	$(function(){
			...
			$.ajax({
				...
				success:function (data){
					if(data.code=="1"){
						...
						//拼接div
						var str="";
						str+="<div id='div_"+data.other.id+"' class='remarkDiv' style='height: 60px;'>";
						...
						str+="		<h5>"+data.other.noteContent+"</h5>";
						str+="		<font color='gray'>市场活动</font> <font color='gray'>-</font> <b>${activity.name}</b> <small style='color: gray;'> "+data.other.createTime+" 由${sessionScope.sessionUser.name}创建</small>";
						str+="		<div style='position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;'>";
						str+="			<a class='myHref' name='editA' remarkId='"+data.other.id+"' href='javascript:void(0);'><span class='glyphicon glyphicon-edit' style='font-size: 20px; color: #E6E6E6;'></span></a>";
						str+="			&nbsp;&nbsp;&nbsp;&nbsp;";
						str+="			<a class='myHref' name='deleteA' remarkId='"+data.other.id+"' href='javascript:void(0);'><span class='glyphicon glyphicon-remove' style='font-size: 20px; color: #E6E6E6;'></span></a>";
						str+="		</div>";
						str+="	</div>";
						str+="</div>";
						$("#remarkDiv").before(str);//在id:remarkDiv标签之外的前面添加
					}else{
						//提示信息
						alert(data.message);
					}
		...
		//给所有的市场活动后面的"修改"图标添加单击事件
		$("#remarkDivList").on("click","a[name='editA']",function (){
			//获取备注的id和noteContent
			var id=$(this).attr("remarkId");
			var noteContent=$("#div_"+id+" h5").text();
			//把备注的id和noteContent写到修改的模态窗口中
			$("#edit-id").val(id);
			...
			$.ajax({
				...
				success:function (data){
					if(data.code=="1"){
						...
						$("#div_"+data.other.id+" h5").text(data.other.noteContent);
						$("#div_"+data.other.id+" small").text(" "+data.other.editTime+" "+"由${sessionScope.sessionUser.name}修改");
					}else{
						//提示信息
						alert(data.message);
						//模态窗口不关闭
